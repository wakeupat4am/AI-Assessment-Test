...
- **Method:** I froze the initial A0/B0 result before improvements. `public.json` contains the 10 published questions used throughout development, `dev.json` contains 20 diagnostic questions used for calibration, and `holdout.json` is a 10-question internal diagnostic split. Per-query rules were not fitted to the holdout, but its aggregate results were inspected during architecture selection; it is therefore not claimed as a blind external test. Retrieval uses organizer gold printed pages and reports Hit@1/5/10. Citation precision/recall compares every cited page with all recorded gold pages; alternative supporting pages are manually accepted only after inspecting the retrieved source. Refusal is scored separately for answerable/unanswerable questions. The automatic answer diagnostic requires complete numeric recall plus token-F1 >= 0.45; it is not exact string matching, but still undercounts concise correct paraphrases. The primary public answer judgement is an auditable per-question manual semantic review. The reported public 100% is a post-calibration regression result; organizer-held questions are the generalization test.
...
- **Results on the 10 published questions:**
  ...
  Across all 40 public/dev/internal-holdout questions, B4b achieved 72.5% manual accuracy, Hit@1/5/10 of 63.9%/83.3%/91.7%, and canonical citation P/R of 63.9%. On the internal holdout, it improved manual accuracy from A3.1 dense-only's 50% to 70%, Hit@1 from 44.4% to 77.8%, and citation overlap from 33.3% to 55.6% at roughly 61 ms retrieval latency. These inspected internal results support engineering decisions but are not a substitute for organizer-held evaluation.
...
The one-take command streams every answer, citation and latency while writing
the same auditable JSONL/summary artifacts as the batch evaluator:

```bash
make demo-financial QUESTIONS=techcombank_rag/data/evaluation/public.json
```

**TODO before submission:** add the 3–5 minute unedited video link showing a clean start and one uninterrupted run over all 10 published questions, including real latency, citations, refusal behavior and any visible failure.
